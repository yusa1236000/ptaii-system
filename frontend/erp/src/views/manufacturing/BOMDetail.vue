...
<template>
    <div class="bom-detail">
      ...
      <div v-else-if="!bom" class="not-found">
        ...
        <h2>BOM Not Found</h2>
        <p>The BOM you're looking for doesn't exist or you don't have permission to view it.</p>
      ...
      <div v-else class="bom-container">
          ...
          <div class="card-body">
            <div class="info-grid">
              ...
              <div class="info-item">
                ...
                <div class="info-value">{{ formatDate(bom.effective_date) }}</div>
              </div>
              <div class="info-item">
                <div class="info-label">Status</div>
                <div class="info-value">
                  <span class="status-badge" :class="getStatusClass(bom.status)">
                    {{ bom.status }}
                  </span>
                </div>
              </div>
              <div class="info-item">
                ...
                <div class="info-value">
                  {{ bom.standard_quantity }} {{ bom.unitOfMeasure?.symbol || '' }}
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="card bom-lines">
          <div class="card-header">
            ...
            <button v-if="bom.status !== 'Obsolete'" class="btn btn-primary btn-sm" @click="openAddLineModal">
          ...
          <div class="card-body">
            ...
            <div v-else-if="bomLines.length === 0" class="empty-lines">
              ...
              <button v-if="bom.status !== 'Obsolete'" class="btn btn-primary" @click="openAddLineModal">
                Add Component
              </button>
            </div>
            ...
            <table v-else class="data-table">
              <thead>
                <tr>
                  ...
                  <th>Critical</th>
                  <th>Notes</th>
                  <th v-if="bom.status !== 'Obsolete'">Actions</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="line in bomLines" :key="line.line_id">
                  ...
                  <td>{{ line.notes || '-' }}</td>
                  <td v-if="bom.status !== 'Obsolete'" class="actions-cell">
                    <button class="action-btn" title="Edit Component" @click="editLine(line)">
                      <i class="fas fa-edit"></i>
                    </button>
                    <button class="action-btn" title="Delete Component" @click="confirmDeleteLine(line)">
  ...
  export default {
    ...
    setup() {
            ...
            bomLines.value = [
              {
                line_id: 1,
                bom_id: bomId,
                item: { item_id: 101, name: 'Motherboard X570', item_code: 'MB-X570' },
                quantity: 1,
                unitOfMeasure: { name: 'Each', symbol: 'EA' },
                is_critical: true,
              ...
              },
              {
                line_id: 2,
                bom_id: bomId,
                item: { item_id: 102, name: 'CPU Ryzen 7', item_code: 'CPU-R7' },
                quantity: 1,
                unitOfMeasure: { name: 'Each', symbol: 'EA' },
                is_critical: true,
                notes: null
              },
              {
                line_id: 3,
                bom_id: bomId,
                item: { item_id: 103, name: 'RAM DDR4 16GB', item_code: 'RAM-16G' },
                quantity: 2,
                unitOfMeasure: { name: 'Each', symbol: 'EA' },
                is_critical: false,
                notes: null
              },
              {
                line_id: 4,
                bom_id: bomId,
                item: { item_id: 104, name: 'SSD 1TB', item_code: 'SSD-1T' },
                quantity: 1,
                unitOfMeasure: { name: 'Each', symbol: 'EA' },
                is_critical: false,
                notes: null
      ...
      const saveLine = async (lineData) => {
        try {
          if (isEditingLine.value) {
            await BOMService.updateBOMLine(bomId, lineData.line_id, lineData);
          } else {
      ...
      const deleteLine = async () => {
        try {
          await BOMService.deleteBOMLine(bomId, lineToDelete.value.line_id);
          // Refresh BOM lines
          fetchBOMLines();
          closeDeleteModal();
        } catch (error) {
