    ...
    <div class="sales-quotations">
      ...
      <div class="quotations-container">
        ...
        <div v-else-if="filteredQuotations.length === 0" class="empty-state">
          ...
          <p>Coba sesuaikan pencarian atau filter, atau buat penawaran baru.</p>
        </div>
        ...
        <div v-else class="quotations-grid">
          ...
          >
            <div class="quotation-header">
              <div class="quotation-status" :class="getStatusClass(quotation.status)">
                {{ getStatusLabel(quotation.status) }}
              </div>
              <div class="quotation-actions">
                <button class="action-btn" title="Edit Penawaran" @click.stop="editQuotation(quotation)">
            ...
            <div class="quotation-content" @click="viewQuotation(quotation)">
              <div class="quotation-id">{{ quotation.quotation_number }}</div>
              <div class="quotation-customer">{{ quotation.customer.name }}</div>
              <div class="quotation-dates">
                ...
                <div class="date-item">
                  ...
                  <span>Valid sampai: {{ formatDate(quotation.validity_date) }}</span>
                </div>
              </div>
              
              <div class="quotation-terms" v-if="quotation.payment_terms || quotation.delivery_terms">
                <div v-if="quotation.payment_terms" class="terms-item">
                  <strong>Syarat Pembayaran:</strong> {{ quotation.payment_terms }}
                </div>
                <div v-if="quotation.delivery_terms" class="terms-item">
                  <strong>Syarat Pengiriman:</strong> {{ quotation.delivery_terms }}
                </div>
              </div>
            </div>
            ...
            <div class="quotation-footer">
              ...
              >
                <i class="fas fa-file-invoice-dollar"></i> Konversi ke SO
              </button>
              <button 
      ...
      <div v-if="filteredQuotations.length > 0" class="pagination">
        <div class="pagination-info">
          Menampilkan {{ paginationInfo.from }} - {{ paginationInfo.to }} dari {{ filteredQuotations.length }} penawaran
        </div>
      ...
      <div v-if="showDeleteModal" class="modal">
        ...
        <div class="modal-content modal-sm">
          ...
          <div class="modal-body">
            <p>Apakah Anda yakin ingin menghapus penawaran <strong>{{ quotationToDelete.quotation_number }}</strong>?</p>
    ...
    setup() {
      ...
      const fetchQuotations = async () => {
        ...
        try {
          ...
          const errorMsg = error.response?.data?.message || 'Terjadi kesalahan saat memuat data penawaran';
          alert(errorMsg);
      ...
      const filteredQuotations = computed(() => {
        ...
        if (statusFilter.value) {
          result = result.filter(quotation => quotation.status === statusFilter.value);
        }
      ...
      const getStatusLabel = (status) => {
        switch (status) {
          ...
          case 'Converted': return 'Dikonversi ke SO';
      ...
      const viewQuotation = (quotation) => {
        router.push(`/quotations/${quotation.quotation_id}`);
      };
      
      const editQuotation = (quotation) => {
        router.push(`/quotations/${quotation.quotation_id}/edit`);
      };
      ...
      const deleteQuotation = async () => {
        try {
          await axios.delete(`/quotations/${quotationToDelete.value.quotation_id}`);
          
          // Remove from local state
          quotations.value = quotations.value.filter(
            q => q.quotation_id !== quotationToDelete.value.quotation_id
          );
          ...
          const errorMsg = error.response?.data?.message || 'Terjadi kesalahan saat menghapus penawaran';
          alert(errorMsg);
      ...
      const markAsRejected = async (quotation) => {
        try {
          ...
          console.error('Error updating quotation:', error);
          const errorMsg = error.response?.data?.message || 'Terjadi kesalahan saat mengubah status penawaran';
          alert(errorMsg);
      ...
      const convertToOrder = (quotation) => {
        router.push(`/orders/create-from-quotation/${quotation.quotation_id}`);
      };
